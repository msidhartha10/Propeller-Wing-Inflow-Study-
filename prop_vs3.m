...
% --- Generate NACA 0012 airfoil shape ---
x_af = linspace(0, 1, 200);   % Normalized chordwise locations
t = 0.12;                     % 12% thickness for NACA 0012
yt = 5 * t * ( ...
    0.2969 * sqrt(x_af) - ...
    0.1260 * x_af - ...
    0.3516 * x_af.^2 + ...
...
% Combine into one airfoil shape (LE to TE)
x_af_comb = [xu, flip(xl(2:end))];           % Leading to trailing
z_af_comb = [yu, flip(yl(2:end))];           % Thickness along Z

% Scale to real dimensions
chord = 0.225;                               % Chord length in meters
x_af_scaled = x_af_comb * chord;             % Place LE at x=0
z_af_scaled = z_af_comb * chord;

% Spanwise extrusion
span_center = 0.075;                         % Lateral offset from root (m)
half_span = 0.5;                             % Half wing span
y_af_span = linspace(span_center - half_span, span_center + half_span, 50);

% Extrude airfoil along span
[X_af, Y_af] = meshgrid(x_af_scaled, y_af_span);
Z_af = repmat(z_af_scaled, length(y_af_span), 1);
...
surf(X_af, Y_af, Z_af, ...
    ...
    'EdgeColor', 'none', ...
    'FaceAlpha', 1.0);  % Opaque for contrast
